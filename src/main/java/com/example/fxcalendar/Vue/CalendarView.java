    ...
    public void displayEventsParSemaine(List<biweekly.component.VEvent> events, LocalDate semaine) {
        ...
        for (int i = 0; i < 5; i++) { // Monday to Friday
            ...
            if (i < 4) {
                ...
                separatorLine.setStartX(0.0);
                separatorLine.setEndX(0.0);
                separatorLine.setStartY(0.0);
                separatorLine.setEndY(1.0); // Line extends full height
                separatorLine.setStrokeWidth(1.0);
                separatorLine.setStroke(Color.BLACK);
                VBox separatorContainer = new VBox(separatorLine);
                separatorContainer.setAlignment(Pos.CENTER);
                calendarGrid.add(separatorContainer, i + 1, 0);
            }
        }
        ...
        for (int i = 8; i <= 19; i++) {
            ...
            VBox hourLabels = new VBox(hourLabel1, hourLabel2);
            hourLabels.setAlignment(Pos.CENTER);
            calendarGrid.add(hourLabels, 0, (i - 8) * 2 + 1);

            // Add separator line between hours
            if (i < 19) {
                Line separatorLine = new Line();
                separatorLine.setStartX(0.0);
                separatorLine.setEndX(1.0); // Line extends full width
                separatorLine.setStartY(0.0);
                separatorLine.setEndY(0.0);
                separatorLine.setStrokeWidth(1.0);
                // Add a small gap between hours
                VBox.setMargin(separatorLine, new Insets(2)); // Adjust as needed
                VBox separatorContainer = new VBox(separatorLine);
                separatorContainer.setAlignment(Pos.CENTER);
                calendarGrid.add(separatorContainer, 0, (i - 8) * 2 + 2);
            }
